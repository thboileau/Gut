...
const thrust = require('../git/thrust');
...
module.exports = (() => {
  const createPullRequest = (shouldCreatePullRequest) => {
    ...
    if (!reviewTool) {
      execution.exit(1, `The review tool ${parsedRepositoryPath.gitServer} is not supported at the moment.\n` +
  ...
  return {
    pr: () => {
      ...
      const branchRemote = branches.getBranchInfo(branches.BRANCH_INFO_PARTS.REMOTE);

      if (!branchRemote) {
        thrust.defaultFirstThrust();
      }
