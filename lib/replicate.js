...
const {
  print, execute, executeAndPipe
} = require('./utils/execution');
const {
  GIT_SERVERS_PRESET, configureGutIfNeeded, getGitServer
} = require('./utils');
        ...
        .then(gutOptions => {
          const ARGUMENTS = {
            SERVER: {
              ...
              choices: _.keys(GIT_SERVERS_PRESET)
            },
            OWNER: {
              name: 'owner',
              alias: 'o',
          ...
          const serverConfiguration = getGitServer(args.server);
          ...
          print(`Cloning ${repositoryUrl} into ${repositoryPath}\n`);

          execute(`mkdir -p ${repositoryOwnerPath}`);
          process.chdir(repositoryOwnerPath);
          executeAndPipe('git', [ 'clone', repositoryUrl ]);
        });
    }
  };
})();
