...
const { configureGutIfNeeded } = require('./utils');
...
module.exports = (() => {
  ...
  const createBranch = (newParsedBranch, gutOptions) => {
    ...
    newBranchDescription.author = gutOptions.accounts.github; // TODO: change when adding new git servers
    newBranchDescription.baseBranch = getCurrentBranch();

    const newBranchName = buildBranchName(newParsedBranch);

    execute(`git checkout -b ${newBranchName}`);
    execute(`git config branch.${newBranchName}.description '${JSON.stringify(newParsedBranch)}'`);
  };

  const createVersionBranch = (parsedBranch, args, gutOptions) => {
    const newVersion = args[ ARGUMENTS.NEW_VERSION_BRANCH.name ];
    if (!isMasterBranch(parsedBranch)) {
    ...
    newParsedBranch.type = BRANCH_TYPES.VERSION.name;
  ...
  const createFeatureBranch = (parsedBranch, args, gutOptions) => {
    ...
    if (!isMasterBranch(parsedBranch) && !isVersionBranch(parsedBranch)) {
    ...
    newParsedBranch.type = BRANCH_TYPES.FEATURE.name;
  ...
  const createDevBranch = (parsedBranch, args, gutOptions) => {
    ...
    if (!isVersionBranch(parsedBranch) && !isFeatureBranch(parsedBranch)) {
    ...
    newParsedBranch.type = BRANCH_TYPES.DEV.name;
    ...
    burgeon: (yargs) => {
      configureGutIfNeeded()
        .then(gutOptions => {
          ...
          const parsedBranch = parseBranchName();
