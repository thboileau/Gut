...
const { getBranchOnlyCommits } = require('./utils/branches');
const {
  print, execute
...
module.exports = (() => {
  ...
  const printLineDiff = parsedDiff => {
    ...
    print(_.join(fragmentsToPrint, ''));
    print('');
  };

  const printOddities = parsedDiff => {
    if (_.isEmpty(parsedDiff.oddities)) {
      print('No oddities found, good job Bob!'.green);
      return;
    }
      ...
      .each(oddityPair => {
        ...
        _(oddities)
          ...
          .each(oddity => {
            ...
            print(`  Found a ${oddity.oddityType.italic}`, warningMessage);
            print(`      ${oddity.lineNumber.toString().magenta}: ${oddity.line.cyan}`);
          });
      });
  };
  ...
  return {
    audit: (yargs, os) => {
      ...
      if (commitsNumber) {
        diff = execute(`git --no-pager diff -U0 --no-color HEAD~${commitsNumber}..HEAD`);
      } else if (fromBaseBranch) {
        const branchOnlyCommits = getBranchOnlyCommits();
        const numberOfCommits = _.size(branchOnlyCommits);
        print(`Number of commits for the current PR: ${numberOfCommits}`.cyan);
        diff = numberOfCommits >= 1
          ? execute(`git --no-pager diff -U0 --no-color HEAD~${numberOfCommits}..HEAD`)
          : '';
      } else if (from) {
        diff = execute(`git --no-pager diff -U0 --no-color ${from}..${to}`);
      } else {
        diff = execute('git --no-pager diff -U0 --no-color');
      }
