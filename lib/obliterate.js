...
const {
  executeAndPipe, print, exit
} = require('./utils/execution');
const { yesNoPrompt } = require('./utils/prompt');
...
module.exports = (() => {
  const REMOTES = {
    LOCAL: {
      ...
      deleteCommands: {
        branch: (branchToDelete) => executeAndPipe('git', [ 'branch', '-D', branchToDelete ]),
        tag: (tagToDelete) => executeAndPipe('git', [ 'tag', '-d', tagToDelete ])
      }
    },
    ORIGIN: {
      name: 'origin'.blue,
      ...
      deleteCommands: {
        branch: (branchToDelete) => executeAndPipe('git', [ 'push', 'origin', '--delete', branchToDelete ]),
        tag: (tagToDelete) => executeAndPipe('git', [ 'push', 'origin', '--delete', tagToDelete ])
      }
    },
    UPSTREAM: {
      name: 'upstream'.red,
      ...
      deleteCommands: {
        branch: (branchToDelete) => executeAndPipe('git', [ 'push', 'upstream', '--delete', branchToDelete ]),
        tag: (tagToDelete) => executeAndPipe('git', [ 'push', 'upstream', '--delete', tagToDelete ])
      }
    }
  };
  ...
  const deleteItem = (itemName, itemToDelete, targetRemoteArgument) => {
    ...
    if (!targetRemote) {
      exit(`No remote found for argument: '${targetRemoteArgument}'`, 1);
    }
    ...
    yesNoPrompt(
      ...
      (value) => {
        if (value) {
          ...
          print('Operation aborted.'.yellow);
        }
      }
    );
  };
