...
const { searchForLocalBranch } = require('./utils/branches');
const {
  execute, print
  ...
  return {
    switch: (yargs) => {
      ...
      if (target) {
        ...
        const matchingBranches = searchForLocalBranch(branchRegex);

        if (_.isEmpty(matchingBranches)) {
          print('No branch found matching the regex'.red);
        } else if (matchingBranches.length === 1) {
          execute(`git checkout ${matchingBranches[ 0 ]}`);
        } else {
          const matchingBranchesAsString = _.join(matchingBranches, '\n');
          print(`The following branches matched the search, please change it so that there's one match:\n${matchingBranchesAsString}`.red);
        }
      }
    }
  };
