...
const { parseBranchName } = require('./utils/branches');
const {
  print, executeAndPipe
} = require('./utils/execution');
const {
  getRepositoryOption, hasUnstagedChanges
} = require('./utils');
  ...
  return {
    execute: yargs => {
      ...
      if (hasUnstagedChanges()) {
        print('You have unstaged changes, you should ideally only work on one subject at once!.'.yellow);
      }
      ...
      const commitMessageSuffixTemplate = getRepositoryOption('commitMessageSuffixTemplate');
      ...
      if (commitMessageSuffixTemplate) {
        ...
        if (parsedBranch.ticketNumber) {
          ...
          const messageWithSuffix = _([ message, suffix ])
            ...
            .join(' ');
          executeAndPipe(`git commit -m '${messageWithSuffix}'`);
        } else {
          print('Commit message suffix template found but the branch does not contain a ticket number, no suffix added.'.yellow);
          executeAndPipe(`git commit -m '${message}'`);
        }
      } else {
        print('No commit message suffix template found, no suffix added.');
        executeAndPipe(`git commit -m '${message}'`);
      }
    }
  };
